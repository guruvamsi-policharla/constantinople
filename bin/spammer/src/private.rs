...
pub async fn run_private(
    ...
    loop {
        ...
        let mut planned: Vec<(usize, PrivateEffect, String)> = Vec::with_capacity(batch_size);
        let mut scanned = 0;
        while batch.len() < batch_size && scanned < n {
            ...
            if let Some((tx, effect)) = plan_and_sign(
                ...
                planned.push((i, effect, tx.message_digest().to_string()));
                batch.push(tx);
            }
        }
        ...
        for (index, effect, digest) in &planned {
            if outcome.finalized(digest) {
