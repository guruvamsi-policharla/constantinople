...
fn maybe_decode_public_key(hex_str: &str) -> Option<ed25519::PublicKey> {
    let bytes = from_hex(hex_str)?;
    ed25519::PublicKey::read(&mut &bytes[..]).ok()
}
...
pub fn load_local_config(peers_path: &Path, config_path: &Path) -> LoadedConfig {
    ...
    let participants = peers
        .validators
        .iter()
        .map(|peer| decode_public_key("peer public_key", &peer.name))
        .collect();
    ...
    decode_with_network(config, public_listen, participants, bootstrappers, false)
}
...
pub fn load_deployer_config(hosts_path: &Path, config_path: &Path) -> LoadedConfig {
    ...
    let participants = hosts
        .hosts
        .iter()
        .filter_map(|host| maybe_decode_public_key(&host.name))
        .collect();
    ...
    decode_with_network(config, public_listen, participants, bootstrappers, true)
}
...
"#,
        ...
        assert_eq!(loaded.decoded.participants.len(), 2);
        assert!(loaded.decoded.participants.contains(&public_key));
        assert!(loaded.decoded.participants.contains(&public_keys[1]));
        assert_eq!(
...
"#,
    ...
    fn deployer_config_tracks_all_participants_not_only_bootstrappers() {
        ...
        let public_key = signer.public_key();
        let share = shares.get(&public_key).expect("missing share");
        let bootstrapper_name = hex(&public_keys[1].encode());
        let third_name = hex(&public_keys[2].encode());
        let self_name = hex(&public_key.encode());
        let config_path = temp_path("validator-config", ".yaml");
        let hosts_path = temp_path("validator-hosts", ".yaml");
...
"#,
        ...
        assert_eq!(loaded.decoded.participants.len(), 3);
        assert!(loaded.decoded.participants.contains(&public_key));
        assert!(loaded.decoded.participants.contains(&public_keys[1]));
        assert!(loaded.decoded.participants.contains(&public_keys[2]));
